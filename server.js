...
app.get('/vendor-b', async (req,res)=>{
    ...
    const insertionPromises = data.map(p => {
        const stockValue = p.isAvailable ? "Tersedia" : "Kosong";
        return pool.query(
            
            `INSERT INTO products(vendor,code,name,price,stock) VALUES($1,$2,$3,$4,$5)`,
            
            ['VendorB', p.sku, p.productName, p.price, stockValue] 
        );
    });
    ...
    try {
        ...
        console.error("Kesalahan Database Saat Insert Vendor B:", error);
        
        res.status(500).json({ error: "Gagal memasukkan data vendor B ke database." });
    }
});
